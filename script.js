...
function checkGuess(e) {
	...
	if (!userGuessesArr.includes(guess)) {
		// check array of all previous guesses to make sure the guess has not been made before
		...
		if (guess.length > 1) {
			// if the user has attempted to guess the entire word
		...
		if (!correctGuess) {
			// if the user made and inccorect guess
		...
		if (userWrongGuesses > maxWrongGuesses) {
			// if user has too many wrong guesses
...
/**********code for modal box if win/lose*************** */
const modal = document.querySelector("#modal");

const close = document.querySelector("#close");

function openModal(message) {
	modal.firstElementChild.firstElementChild.innerText = message; // display the correct message either you win or you lose
	modal.style.display = "block";
}

function closeModal() {
	modal.style.display = "none"; // function attached to close button on modal box
}

close.addEventListener("click", closeModal); // event listener for closing modal once opened
/************************end of modal box code******************/

submitWordButton.addEventListener("click", createBoard); // add event listener to the first submit button
